...
public class ClientIdUpperCaseUDPBurst {
  ...
  private List<String> launch() throws IOException {
    try {
      ...
      return Arrays.stream(upperCaseLines).toList();
    } finally {
      dc.close();
    }
  }
  ...
  private void doRead() throws IOException {
    ...
    if (sender == null){
      logger.warning("Le packet n'a pas été reçu");
      return;
    }
    receiveBuffer.flip();
    ...
    if (id < 0){
      logger.info("Le packet reçu n'est pas le bon, l'id est négative");
      return;
    }
    if (!receiveBuffer.hasRemaining()){
      logger.info("Le packet reçu n'est pas le bon, il y a pas de message");
      return;
    }
    if (bitSet.get((int) id)){
      logger.info("Le packet a déjà été reçu");
      return;
    }
    bitSet.set((int) id);
